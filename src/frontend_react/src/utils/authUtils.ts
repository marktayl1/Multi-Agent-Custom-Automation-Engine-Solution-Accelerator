import { AuthHeaders, UserClaim, UserDetails } from '../types';
...
export const getAuthDetails = async (): Promise<AuthHeaders | null> => {
  ...
  if (window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
    ...
    try {
      ...
      const userDetails: UserDetails = authData[0] || {};

      // Find user claims
      const findClaim = (type: string): string => 
        userDetails.user_claims?.find((claim: UserClaim) => claim.typ === type)?.val || '';

      // Construct headers using the global config object
      const headers: AuthHeaders = {
        'Content-Type': 'application/json',
        'X-Ms-Client-Principal': userDetails?.client_principal || '',
        'X-Ms-Client-Principal-Id': findClaim('http://schemas.microsoft.com/identity/claims/objectidentifier'),
        'X-Ms-Client-Principal-Name': findClaim('name'),

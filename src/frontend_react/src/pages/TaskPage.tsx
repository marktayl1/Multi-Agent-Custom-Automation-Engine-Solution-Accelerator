...
import { TaskPageProps, TaskMessage, TaskStage, TaskAgent } from '../types';
...
interface ProcessedTaskMessage extends TaskMessage {
  timestamp: Date;
}

const TaskPage: React.FC<TaskPageProps> = ({ onOpenAgentsModal, onOpenWorkflowModal, onOpenHistoryModal }) => {
  ...
  const [messages, setMessages] = useState<ProcessedTaskMessage[]>([]);
  const [newMessage, setNewMessage] = useState<string>('');
  const [charCount, setCharCount] = useState<number>(0);
  const [agentsInfo, setAgentsInfo] = useState<AgentsInfo>({ total: 0, human: 0 });
  ...
  const fetchTaskMessages = async (sessionId: string): Promise<void> => {
    try {
      ...
      if (data && Array.isArray(data)) {
        // Process messages
        const formattedMessages: ProcessedTaskMessage[] = data.map(msg => {
  ...
  const fetchTaskStages = async (sessionId: string): Promise<void> => {
    try {
            ...
            <ul className="menu-list">
              <li>
                <ul id="taskStagesMenu" className="menu-stages">
                  {taskData && taskData.steps && taskData.steps.map((stage: TaskStage, index: number) => (
